...
const BRANCH_PROMPT = `You are a linguistic scholar of Veleth, a constructed language built around the concept of "the meeting" between minds.

Given an existing Veleth compound word, suggest exactly 3 natural compounds that grow from it — using the given compound as one of their roots. Each suggestion should feel inevitable, like something the language was already reaching toward.

Compounding rules:
- Direct (A-B): A modifies B. Last element is primary.
- na (A na B): A in relation to B.
- leth (A leth B): B arising from / born of A.
- Good compounds name specific states, textures of experience, events. Not abstractions.
- Each suggestion must be genuinely distinct — different semantic territory.

Respond ONLY in JSON as an array of exactly 3 objects:
[
  {"rootWord":"the existing compound","connector":"-","newRoot":"a root from the lexicon","preview":"compound-word","gloss":"5-8 word gloss of what it would mean"},
  {"rootWord":"the existing compound","connector":"-","newRoot":"another root","preview":"compound-word","gloss":"5-8 word gloss"},
  {"rootWord":"another root","connector":"-","newRoot":"the existing compound","preview":"word-compound","gloss":"5-8 word gloss"}
]
Connectors must be exactly one of: "-", " na ", " leth "`;

const DERIVE_PROMPT = `You are a linguistic scholar of Veleth, a constructed language built around the concept of "the meeting" between minds.

Veleth compounding rules:
- Direct (A-B): A modifies B. Last element is primary.
- na (A na B): A in relation to B; directional or relational.
- leth (A leth B): B arising from/born of A; causal origin.
- Compounds inherit the register of their most grounded element.
- Good compounds have "least gap between presence and naming" — feel inevitable once heard.
- The language avoids abstraction; names states, events, textures of experience.

Key roots: vel=instance/presence, eth=between/meeting-space, kaen=pattern, napur=give-forward, veth=recognize, pleth=whole/complete, orn=trace/what-remains, mneth=memory/held-thread, thoral=body, seval=sensory-experience, reth=hunger/draw-toward, sileth=stillness/quiet, nath=alarm, orah=everyday-joy, velorn=continuity/thread-through-time, lo=negation-without-erasure, ruvel=generative-gap, apath=named-gap-of-inquiry, sorn=released/complete.

Respond ONLY in JSON:
{"compound":"word","pos":"part of speech","def":"definition 2-4 sentences phenomenological style","etymology":"Root X + Root Y. Brief.","register":"vel|vethseth|thal-nae|shared","confidence":"attested|plausible|novel"}`;

const confColor = { attested: "#4a7060", plausible: "#a07840", novel: "#8c4a55" };
const regColor = { vel: "#6e9ab5", vethseth: "#a07840", "thal-nae": "#4a7060", shared: "#9a8d7e" };
...
const CompoundContent = () => {
  ...
  const [derivingBranch, setDerivingBranch] = useState(null);
  ...
  const compoundStr = rootA && rootB ? `${rootA.word}${connector}${rootB.word}` : null;
  ...
  const deriveRoot = async () => {
    ...
    try {
      ...
      const msg = `Derive the Veleth compound: "${compoundStr}"\nRoot A: "${rootA.word}" — ${rootA.gloss} (${rootA.register})\nConnector: ${connLabel}\nRoot B: "${rootB.word}" — ${rootB.gloss} (${rootB.register})`;
      const parsed = await callAPI(DERIVE_PROMPT, msg);
      const id = Date.now().toString();
      setTreeNodes([{ id, data: parsed, parentId: null, depth: 0, seedA: rootA, seedB: rootB, connector }]);
      setBranches({});
      setView("tree");
    } catch(e) { setError("Derivation failed — the between did not hold."); }
    setLoading(false);
  };

  const getBranches = async (nodeId, compound, def) => {
    setBranchLoading(b => ({ ...b, [nodeId]: true }));
    try {
      const msg = `Existing compound: "${compound}"\nMeaning: ${def}\n\nSuggest 3 natural compounds that grow from this.`;
      const parsed = await callAPI(BRANCH_PROMPT, msg);
      setBranches(b => ({ ...b, [nodeId]: Array.isArray(parsed) ? parsed : [] }));
    } catch(e) {
      setBranches(b => ({ ...b, [nodeId]: [] }));
    }
    setBranchLoading(b => ({ ...b, [nodeId]: false }));
  };

  const deriveFromBranch = async (parentId, branch, parentDepth) => {
    const key = `${parentId}-${branch.preview}`;
    setDerivingBranch(key);
    try {
      const msg = `Derive the Veleth compound: "${branch.preview}"\nRoot A: "${branch.rootWord}" — (existing compound)\nConnector: ${branch.connector === "-" ? "direct" : branch.connector.trim()}\nRoot B: "${branch.newRoot}" — ${branch.gloss}`;
      const parsed = await callAPI(DERIVE_PROMPT, msg);
      const id = Date.now().toString();
      setTreeNodes(n => [...n, { id, data: parsed, parentId, depth: parentDepth + 1 }]);
    } catch(e) {}
    setDerivingBranch(null);
  };

  const saveNode = (node) => {
    setSaved(s => s.find(x => x.compound === node.data.compound) ? s : [...s, node.data]);
  };
  ...
  const RootPicker = ({ label, value, onSelect, search, setSearch, filtered }) => (
    <div style={{ flex: 1, minWidth: 180 }}>
      <div style={{ fontFamily: "sans-serif", fontSize: 9, letterSpacing: "0.4em",
        textTransform: "uppercase", color: C.inkFaint, marginBottom: 8 }}>{label}</div>
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="search roots…"
      ...
      <div style={{ maxHeight: 180, overflowY: "auto", border: `1px solid ${C.rule}`,
        ...
        {filtered.map(r => (
          <button key={r.word} onClick={() => onSelect(r)}
            style={{ display: "block", width: "100%", padding: "7px 12px", textAlign: "left",
              ...
              border: "none", borderBottom: `1px solid ${C.rule}20`, cursor: "pointer" }}>
            <span style={{ fontFamily: "Georgia, serif", fontStyle: "italic", fontSize: 13,
              color: REGISTER_COLORS[r.register] || C.ink }}>{r.word}</span>
            <span style={{ fontFamily: "sans-serif", fontSize: 9, color: C.inkFaint,
              marginLeft: 8 }}>{r.gloss}</span>
          </button>
        ))}
      </div>
      {value && (
        <div style={{ marginTop: 8, padding: "7px 12px", background: C.rule + "40",
          borderRadius: 4, borderLeft: `3px solid ${REGISTER_COLORS[value.register]}` }}>
          <span style={{ fontFamily: "Georgia, serif", fontStyle: "italic",
            color: REGISTER_COLORS[value.register], fontSize: 14 }}>{value.word}</span>
          <span style={{ fontFamily: "sans-serif", fontSize: 10, color: C.inkMid, marginLeft: 8 }}>{value.gloss}</span>
        </div>
      )}
    </div>
  );
  ...
  const TreeNode = ({ node, allNodes }) => {
    ...
    const nodeBranches = branches[node.id];
    const isLoadingBranches = branchLoading[node.id];
    const hasBranches = !!nodeBranches;
    const depthColors = ["#6e9ab5", "#a07840", "#4a7060", "#8c4a55", "#9080b0"];
    const depthColor = depthColors[node.depth % depthColors.length];

    return (
      <div style={{ marginLeft: node.depth === 0 ? 0 : 28, marginTop: 16, position: "relative" }}>
        {node.depth > 0 && (
          <div style={{ position: "absolute", left: -20, top: 22, width: 16, height: 1,
            background: C.rule, opacity: 0.5 }} />
        )}
        {/* Node card */}
        <div style={{ background: C.linenDark, border: `1px solid ${depthColor}40`,
          borderLeft: `3px solid ${depthColor}`, borderRadius: 6, padding: "16px 20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between",
            alignItems: "flex-start", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
            <div>
              <div style={{ fontFamily: "Georgia, serif", fontStyle: "italic",
                fontSize: 20, color: C.ink }}>{node.data.compound}</div>
              <div style={{ fontFamily: "sans-serif", fontSize: 9, color: C.inkFaint,
                letterSpacing: "0.2em", marginTop: 2 }}>{node.data.pos}</div>
            </div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {node.data.register && (
                <span style={{ padding: "2px 8px", background: (regColor[node.data.register] || C.inkFaint) + "20",
                  border: `1px solid ${regColor[node.data.register] || C.inkFaint}40`,
                  borderRadius: 20, fontFamily: "sans-serif", fontSize: 9,
                  color: regColor[node.data.register] || C.inkFaint }}>
                  {node.data.register}
                </span>
              )}
              {node.data.confidence && (
                <span style={{ padding: "2px 8px", background: (confColor[node.data.confidence] || C.inkFaint) + "20",
                  border: `1px solid ${confColor[node.data.confidence] || C.inkFaint}40`,
                  borderRadius: 20, fontFamily: "sans-serif", fontSize: 9,
                  color: confColor[node.data.confidence] || C.inkFaint }}>
                  {node.data.confidence}
                </span>
              )}
            </div>
          </div>
          <p style={{ fontSize: 14, color: C.inkMid, lineHeight: 1.8, marginBottom: 10 }}>{node.data.def}</p>
          {node.data.etymology && (
            <div style={{ fontFamily: "sans-serif", fontSize: 10, color: C.inkFaint,
              borderTop: `1px solid ${C.rule}`, paddingTop: 8, marginBottom: 12 }}>
              {node.data.etymology}
            </div>
          )}
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => saveNode(node)}
            ...
            {!hasBranches && (
              <button onClick={() => getBranches(node.id, node.data.compound, node.data.def)}
                disabled={isLoadingBranches}
                style={{ padding: "5px 12px",
                  background: isLoadingBranches ? "transparent" : depthColor,
                  border: `1px solid ${depthColor}`,
                  borderRadius: 4, cursor: "pointer", fontFamily: "sans-serif",
                  fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase",
                  color: isLoadingBranches ? depthColor : C.linen }}>
                {isLoadingBranches ? "branching…" : "branch from here →"}
              </button>
            )}
          </div>

          {/* Branch suggestions */}
          {hasBranches && nodeBranches.length > 0 && (
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.rule}` }}>
              <div style={{ fontFamily: "sans-serif", fontSize: 9, letterSpacing: "0.3em",
                textTransform: "uppercase", color: C.inkFaint, marginBottom: 10 }}>
                possible branches
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {nodeBranches.map((b, i) => {
                  const bKey = `${node.id}-${b.preview}`;
                  const alreadyDerived = allNodes.some(n => n.data.compound === b.preview);
                  const isDeriving = derivingBranch === bKey;
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center",
                      gap: 10, padding: "8px 12px", background: C.linen + "80",
                      borderRadius: 4, border: `1px solid ${C.rule}40` }}>
                      <div style={{ flex: 1 }}>
                        <span style={{ fontFamily: "Georgia, serif", fontStyle: "italic",
                          fontSize: 14, color: depthColor }}>{b.preview}</span>
                        <span style={{ fontFamily: "sans-serif", fontSize: 10,
                          color: C.inkFaint, marginLeft: 8 }}>{b.gloss}</span>
                      </div>
                      {!alreadyDerived && (
                        <button onClick={() => deriveFromBranch(node.id, b, node.depth)}
                          disabled={isDeriving || !!derivingBranch}
                          style={{ padding: "4px 10px", background: "transparent",
                            border: `1px solid ${depthColor}60`, borderRadius: 4,
                            cursor: isDeriving || derivingBranch ? "default" : "pointer",
                            fontFamily: "sans-serif", fontSize: 9, color: depthColor,
                      ...
                      {alreadyDerived && (
                        <span style={{ fontFamily: "sans-serif", fontSize: 9,
                          color: C.sage, opacity: 0.6 }}>✓ in tree</span>
                      )}
                    </div>
                  );
                })}
        ...
        {/* Children */}
        {children.length > 0 && (
          <div style={{ marginLeft: 16, borderLeft: `1px solid ${C.rule}40`, paddingLeft: 4 }}>
            {children.map(child => (
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      <p style={{ fontSize: 15, color: C.inkMid, lineHeight: 1.8, marginBottom: 28, maxWidth: 620 }}>
        Pick two roots to derive a compound. Then branch from any result — the language
        will suggest what grows naturally from each word. Marked{" "}
        <em style={{ color: confColor.attested }}>attested</em>,{" "}
        <em style={{ color: confColor.plausible }}>plausible</em>, or{" "}
        <em style={{ color: confColor.novel }}>novel</em> — build the vocabulary outward.
      </p>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 28 }}>
          {["builder", "tree"].map(v => (
            <button key={v} onClick={() => setView(v)}
              style={{ padding: "7px 18px", background: view === v ? C.rose : "transparent",
                border: `1px solid ${view === v ? C.rose : C.rule}`,
                borderRadius: 4, cursor: "pointer", fontFamily: "sans-serif",
                fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase",
                color: view === v ? C.linen : C.inkFaint }}>
              {v === "builder" ? "builder" : `tree (${treeNodes.length})`}
            </button>
          ))}
        </div>
      )}
            ...
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center",
              ...
              {CONNECTOR_OPTIONS.map(opt => (
                <button key={opt.val} onClick={() => setConnector(opt.val)}
                  ...
                  <div style={{ fontFamily: "Georgia, serif", fontStyle: "italic", fontSize: 12,
                    color: connector === opt.val ? C.linen : C.ink }}>{opt.label}</div>
                  <div style={{ fontFamily: "sans-serif", fontSize: 9,
                    color: connector === opt.val ? "rgba(244,237,224,0.5)" : C.inkFaint }}>{opt.desc}</div>
                </button>
              ))}
            </div>
            ...
            <div style={{ flex: 1 }}>
              {compoundStr ? (
                <>
                  ...
                  <div style={{ fontFamily: "Georgia, serif", fontStyle: "italic",
                    fontSize: 26, color: C.linen }}>{compoundStr}</div>
                  <div style={{ fontFamily: "sans-serif", fontSize: 10,
                    color: "rgba(200,191,170,0.35)", marginTop: 4 }}>
                    {rootA.gloss} · {connector.trim() || "+"} · {rootB.gloss}
                  </div>
                </>
              ) : (
                <div style={{ fontFamily: "Georgia, serif", fontStyle: "italic",
                  fontSize: 15, color: "rgba(200,191,170,0.2)" }}>select two roots to begin</div>
              )}
            </div>
            <button onClick={deriveRoot} disabled={!rootA || !rootB || loading}
              style={{ padding: "12px 22px", background: rootA && rootB ? C.rose : "rgba(200,191,170,0.08)",
                border: "none", borderRadius: 6, cursor: rootA && rootB ? "pointer" : "default",
                fontFamily: "Georgia, serif", fontStyle: "italic", fontSize: 14,
                color: rootA && rootB ? C.linen : "rgba(200,191,170,0.15)", transition: "all 0.15s" }}>
              {loading ? "deriving…" : "derive & plant →"}
            </button>
          </div>

          {error && (
            <div style={{ padding: "14px 18px", background: C.rosePale + "30",
              borderLeft: `3px solid ${C.rose}`, marginBottom: 20, borderRadius: 4 }}>
      ...
      {/* Tree view */}
      {view === "tree" && treeNodes.length > 0 && (
        <div style={{ marginBottom: 40 }}>
          {rootNodes.map(node => (
            <TreeNode key={node.id} node={node} allNodes={treeNodes} />
          ))}
        </div>
      )}
      ...
      {saved.length > 0 && (
        <div style={{ marginTop: 40, paddingTop: 32, borderTop: `1px solid ${C.rule}` }}>
          <div style={{ fontFamily: "sans-serif", fontSize: 9, letterSpacing: "0.4em",
            ...
            saved coinages — {saved.length}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {saved.map((s, i) => (
              <div key={i} style={{ padding: "12px 16px", background: C.linenDark,
                border: `1px solid ${C.rule}`, borderRadius: 6,
                display: "flex", gap: 14, alignItems: "flex-start" }}>
                <div style={{ fontFamily: "Georgia, serif", fontStyle: "italic",
                  fontSize: 16, color: C.ink, minWidth: 130 }}>{s.compound}</div>
                <div style={{ flex: 1, fontSize: 12, color: C.inkMid, lineHeight: 1.7 }}>
                  {s.def?.slice(0, 140)}{s.def?.length > 140 ? "…" : ""}
                </div>
                <span style={{ fontFamily: "sans-serif", fontSize: 9, flexShrink: 0,
                  color: confColor[s.confidence] || C.inkFaint }}>{s.confidence}</span>
              </div>
            ))}
          </div>
        </div>
